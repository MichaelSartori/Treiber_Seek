...
SeekCam::SeekCam(int vendor_id, int product_id, uint16_t* buffer, size_t raw_height, size_t raw_width, cv::Rect roi, std::string ffc_filename, int id) :
    m_offset(0x4000),
    m_ffc_filename(ffc_filename),
    m_is_opened(false),
    m_dev(vendor_id, product_id, id),
    m_raw_data(buffer),
...
std::vector<uint8_t> SeekCam::chip_id()
{
    return m_chip_id;
}